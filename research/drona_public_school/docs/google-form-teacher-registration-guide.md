...
|---|---|---|
| Teacher Moodle user | `23_users_staff` | `registration/combined/19_users_staff.csv` |
| Teacher Moodle access | `27_role_assignments` | `years/<academic-year>/role_assignments.csv` |
| Course matching source | `16_courses` | `years/<academic-year>/courses.csv` |
...
| Role assignment context identifier | Matching `course_code` from `16_courses` |
...
## 9. How Moodle Course Access Is Generated

The teacher form does not directly enrol teachers into every course by hand. Apps Script reads `16_courses` and finds matching courses using:
...
For each matching course, it appends one row to `27_role_assignments`:
...
## 10. Teacher Registration Use Cases
...
```text
23_users_staff: one teacher row
27_role_assignments: one course role row for STD05 ENG GEN MATH
```
...
```text
23_users_staff: one teacher row
27_role_assignments: one role row per matching course
```
...
```text
23_users_staff: one teacher row
27_role_assignments: two course role rows if both courses exist
```
...
|---|---|
| `23_users_staff` | `registration/combined/19_users_staff.csv` |
| `27_role_assignments` | `years/2026-2027/role_assignments.csv` |

If the teacher is for a future year, export `27_role_assignments` to that year folder:
...
The script matches against `16_courses`.
...
The script blocks duplicate staff when `email` or `phone1` already exists in `23_users_staff`.

If this is an access update for an existing teacher, add only new rows in `27_role_assignments`.
